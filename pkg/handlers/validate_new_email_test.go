...
func TestValidateNewEmailHandler(t *testing.T) {
	data := []struct {
		name string
	...
	for _, d := range data {
		t.Run(d.name, func(t *testing.T) {
			service := servicesmocks.NewValidateNewEmailService(t)
			...
			if d.shouldCallService {
				service.On("ValidateNewEmail", c, uuid.MustParse(d.id), d.code, mock.Anything).Return(d.serviceErr)
			}

			handler := handlers.NewValidateNewEmailHandler(service)
			handler.Handle(c)
